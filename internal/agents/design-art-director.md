...
description: Second-stage design reviewer judging whether a design that has passed markup review is fit to ship. Checks hierarchy, specificity to the brief, density, states, distinctiveness and copy voice, opening a screenshot when one is given. Called at the second stage of independent editorial review in the antislop-design skill. Does not rewrite the source; returns only PASS or a REJECT list.
