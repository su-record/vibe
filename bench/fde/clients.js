...
export async function runClient({ client, settings, workspace, harness, env, prompt, timeoutMs, diagnostics, artifactId, captureProcess = capture }) {
  ...
  const errorCode = result.errorCode ?? (!complete ? 'CLIENT_TRANSPORT_INCOMPLETE' : null) ?? (result.exit !== 0 ? 'CLIENT_EXIT_NONZERO' : parsed.errors.length ? 'CLIENT_RESULT_ERROR' : null);
  return { ...parsed, requestedModel: settings.model, exit: result.exit, signal: result.signal, error: errorCode, errorCode,
    transport: result.transport, diagnostics: result.diagnostics, complete, ms: Date.now() - started,
    ...(!complete ? { observedUsage: { tokens: parsed.tokens, models: parsed.models, costUsd: parsed.costUsd, complete: false }, tokens: null, models: [], costUsd: null } : {}),
    usage: !complete ? 'incomplete' : parsed.tokens ? 'captured' : 'missing' };
}
