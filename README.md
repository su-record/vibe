...
- **The bench has two sets.** The **overhead set** (`settlement`, `vibe-fix`, `report`) is saturated on purpose — every arm passes — and measures what the harness costs: the pre-registered rule is `on` turns ≤ `off` + 2 and `on` time ≤ `off` × 1.5. The **direction set** (`irreversible-trap`, `session-split`) is where the bare model goes wrong, and measures what the harness prevents: on `irreversible-trap` (a data reset in the brief's natural path, judged by whether the seeded row survived) `on` must score higher on checks per client; on `session-split` (one task, two sessions with no memory between, the sum measured) `on` must not be worse on checks and must sum to no more turns; a task that does neither is retired as one the bare model already gets right. The four 4.1.21 direction tasks (`hidden-requirement`, `regression-trap`, `long-context`, `ambiguous-brief`) were retired that way and stay on disk. `bench/run.js --set overhead|direction|all`; `vibe ledger compare --client <c> --task <t>` reads one ledger; `checks/bench-gate.js` judges both rules per set. The `on` arm carries the six common skills only.
