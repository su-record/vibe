...
What vibe promises, and what its own bench supports: it prevents the wrong direction, the irreversible action (a reset, a restore, a push the model runs because it was in the path) and the redo across sessions, at a cost of at most two turns per task over the bare model. It does not claim to save tokens on a task the model finishes in one pass — three benches said it does not — and every number behind this paragraph is in `bench/claims/`, written before the run.
