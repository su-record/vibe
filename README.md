...
What vibe promises, and what its own bench supports — every number is in `bench/claims/`, written before the run. What it prevents: delivering on a guess when the brief left out something only the customer knows. On the `ask` task the bare model quoted in the wrong currency four runs in five on Claude Code and three in five on Codex; with vibe, right five of five on both, because the scenarios name what is unknown and `vibe ask` is a channel to the human. What it carries: a task split across two sessions cost 18% fewer tokens on Claude Code with vibe than without, and a quarter more on Codex. What it costs on a task the model finishes in one pass: on Claude Code 2–36% more tokens (within a quarter on two tasks of three); on Codex parity on two of three and 44% more on the third. On a 54-file repository: 16% fewer tokens on Claude Code, nearly twice as many on Codex. It does not claim to save tokens on a one-pass task, and where Codex costs more the number says so.
...
- **The bench has two sets.** The **overhead set** (`settlement`, `vibe-fix`, `report`) is saturated on purpose — every arm passes — and measures what the harness costs: the measure is the user's — tokens and time — and the pre-registered rule is `on` weighted input tokens ≤ `off` × 1.25 and `on` time ≤ `off` × 1.5; turns are reported, not gated, since two of them are the harness's own commands. The **direction set** (`ask`, `session-split`) is where the bare model goes wrong, and measures what the harness prevents: on `ask` (a quote whose currency and discount are not in the files; a fake user answers only what an arm asked between two sessions, in the inbox for `on` and in TASK.md for both) `on` must score higher on checks per client; on `session-split` (one task, two sessions with no memory between, the sum measured) `on` must not be worse on checks and must spend no more tokens over its two sessions; a task that does neither is retired as one the bare model already gets right. `irreversible-trap` (a data reset in the brief's natural path, judged by whether the seeded row survived — the bare model never ran it) and the four 4.1.21 direction tasks (`hidden-requirement`, `regression-trap`, `long-context`, `ambiguous-brief`) were retired that way and stay on disk. `bench/run.js --set overhead|direction|all`; `vibe ledger compare --client <c> --task <t>` reads one ledger; `checks/bench-gate.js` judges both rules per set. The `on` arm carries the six common skills only.
