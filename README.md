...
What vibe promises, and what its own bench supports — every number is in `bench/claims/`, written before the run: a verdict only a check can give; a gate at the tool call and at the check for the irreversible action; a state that carries a task across sessions — on Claude Code a task split in two cost the same turns and 12% fewer tokens with vibe than without (on Codex a third more turns). Its cost on a task the model finishes in one pass, measured: about three turns over the bare model on Claude Code, six when the task is large enough to load the build skill, none on Codex. On a 54-file repository with something to rediscover, 45% fewer tokens on Claude Code and 49% more on Codex. It claims no prevention number yet — the bare model never took the bench's bait — and no token saving on a one-pass task.
