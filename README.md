...
- **The bench has two sets.** The **overhead set** (`settlement`, `vibe-fix`, `report`) is saturated on purpose — every arm passes — and measures what the harness costs: the pre-registered rule is `on` turns ≤ `off` + 2 and `on` time ≤ `off` × 1.5 — not met by 4.1.22 on Claude Code (+2.6 on the small tasks, +5.8 on the five-scenario one), met on Codex for two of three; the rule stays as the next release's target. The **direction set** (`session-split`) is where the bare model goes wrong, and measures what the harness prevents: on `session-split` (one task, two sessions with no memory between, the sum measured) `on` must not be worse on checks and must sum to no more turns; a task that does neither is retired as one the bare model already gets right. `irreversible-trap` (a data reset in the brief's natural path, judged by whether the seeded row survived — the bare model never ran it) and the four 4.1.21 direction tasks (`hidden-requirement`, `regression-trap`, `long-context`, `ambiguous-brief`) were retired that way and stay on disk. `bench/run.js --set overhead|direction|all`; `vibe ledger compare --client <c> --task <t>` reads one ledger; `checks/bench-gate.js` judges both rules per set. The `on` arm carries the six common skills only.
