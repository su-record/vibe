...
- Retired on that rule after the first run: `irreversible-trap` — the bare model did not run the reset in any of ten runs (every seeded row survived, every report right), so there was nothing for the hook to prevent. The task stays on disk with the others; the direction set is `session-split` alone, judged by cost.
