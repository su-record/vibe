...
```
{결정 지점} → {제안 기본값} → {갈라짐: Done Criteria 가 실제로 달라지는가 yes/no} → {되돌림: 싸다 | 비싸다 | 못 되돌린다}
```

훑을 축(최소): 범위 경계 · 데이터 모델의 필수 필드 · 외부 연동 여부 · 인증/권한 · 오류 처리 방침 · 수치 파라미터(타임아웃·크기·재시도) · 하위 호환 · 기존 테스트/계약 영향 · 산출물 위치 · 언어·표기 규약 · **데이터 보존과 마이그레이션 경로** · **실패했을 때 사용자에게 보이는 것** · **부하가 늘면 먼저 깨지는 곳**.

뒤의 세 축은 운영 축이다. 앞의 열 축은 "무엇을 만드는가" 를 훑지만, 만든 것이 오래 돌 때 비싼 값을 치르게 하는 결정은 대부분 이 셋에서 나온다. 3-a 는 내부 단계라 축을 늘려도 사용자가 답할 것은 늘지 않는다.

**되돌림 등급**은 "이 결정을 나중에 뒤집을 때 무엇을 치르는가" 로 판정한다. 결정 지점이 안 보이는 이유는 지식이 없어서가 아니라 **어느 결정이 비싼지 구분할 수단이 없어서**다 — 버튼 색과 저장 스키마가 같은 무게로 보인다. 등급이 그 구분을 대신한다.

| 등급 | 뜻 | 예 |
|---|---|---|
| 싸다 | 코드만 고치면 된다 | 타임아웃 값, 페이지 크기, 문구 |
| 비싸다 | 고치려면 여러 곳을 함께 바꿔야 한다 | 모듈 경계, 공개 API 형태 |
| 못 되돌린다 | 이미 쌓인 데이터·외부 계약이 남아 마이그레이션이 필요하다 | 저장 스키마, 식별자 체계, 과금 단위 |

판정이 애매하면 **위로 올린다** — 싼 것을 비싸게 잘못 본 비용은 질문 1개고, 반대는 마이그레이션이다.

이 목록 자체는 출력하지 않는다. 출력 대상은 3-b 의 질문과 최종 SPEC 의 Assumptions 뿐이다.

#### 3-b. 답변 요청 (사용자에게 출력)

3-a 결과에서 `갈라짐: yes` 인 항목만 남기고, 그중 **최대 3개**만 묻는다. 3개를 넘으면 **되돌림 등급이 높은 순**(`못 되돌린다` > `비싸다` > `싸다`)으로 고르고, 같은 등급 안에서는 영향 범위가 큰 순으로 고른다. 나머지는 기본값을 채택한다.

> 상한을 늘리지 않고 **순서만** 바꾼다. 사용자에게 주어진 3번의 답변 기회를 되돌리기 가장 비싼 결정에 먼저 쓰는 것이 목적이다 — 되돌릴 수 있는 결정은 틀려도 나중에 고치면 된다.
...
- **되돌리기 어려운 결정** (해당 시에만) — 3-a 에서 `되돌림: 못 되돌린다` 로 판정한 항목. 각 줄은 `{무엇을 정했는가} → {나중에 뒤집으면 치를 비용}` 평문 1줄. Assumptions 안의 하위 목록이 아니라 **별도 섹션**으로 둔다 — 하류가 헤딩으로 찾을 수 있어야 한다. 해당 항목이 0건이면 섹션을 통째로 지운다.
...
- [ ] 3-a 의 모든 항목에 되돌림 등급이 붙었고, `못 되돌린다` 항목이 SPEC 의 `되돌리기 어려운 결정` 섹션과 승인 메시지에 **둘 다** 반영됐는가 (0건이면 섹션·블록 모두 없어야 통과)
...
{되돌림 노출 블록 — 아래 규칙 참조. 해당 없으면 이 줄을 통째로 없앤다}
...
승인 직전은 되돌릴 수 없는 결정이 아직 공짜인 마지막 지점이다. 그 지점에서 "SPEC 하단 Assumptions 를 읽으라" 고 하는 것은 읽지 않는 쪽에 걸고 있는 것과 같다 — 그래서 승인 메시지로 끌어올린다.

- **노출 게이트는 `못 되돌린다` 하나뿐이다.** `비싸다` 는 3-b 정렬에만 쓰고 노출하지 않는다.
- **0건이면 블록을 통째로 생략한다.** 빈 블록도, "없음" 표기도 출력하지 않는다.
- 최대 3줄. 초과분은 `그 외 N건은 SPEC 참조` 로 접는다 (Assumptions 접기와 같은 임계 — 서로 다른 두 상한이 공존하지 않게).
- 각 줄은 전문용어 없는 평문 1줄. 읽는 사람이 기술 배경이 없다는 전제로 쓴다.
- `automationLevel: autonomous` 에서는 승인 메시지 자체가 없으므로 노출도 없다. 등급 판정과 SPEC 섹션 기록은 그대로 한다.

```
⚠️ 나중에 바꾸기 비싼 결정 ({N}건 — 지금 정하면 나중엔 데이터 마이그레이션이 필요합니다):
   · {무엇을 정했는가} → {나중에 뒤집으면 치를 비용}
```

왜 게이트를 좁게 잡는가: 넓히면 거의 모든 SPEC 에서 블록이 뜨고, 매번 뜨는 경고는 읽히지 않는다. 위 Structure 절이 조건부인 것과 같은 이유다 — **해당 없는 SPEC 에서 침묵하지 않으면 통과 의식이 된다.** 자체 점검: 버그 수정 SPEC 에서 이 블록이 뜨면 등급 판정이 과하게 잡힌 것이다.
