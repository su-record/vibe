...
1. Turn the discovery result into scenarios. A scenario is stored only if it carries exactly one check type:
   ...
   - When the code will be read or maintained by someone else (a library, a contribution, a handoff), propose `check: { type: review, pack: code, path: <file or directory>, contract: <brief> }`, same mark; otherwise `vibe size` stays the code gate.
