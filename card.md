...
2. Done is only what `vibe check --all` says: build all first, one `check --all`; on a failure, `vibe context <id>` then `check <id>`. Never claim it.
...
9. Report what changed beyond what `vibe check` verified; unchecked is unverified. Keep only the caveat that moves the next step. An error: cause and fix. Answer first; no praise or offer.
