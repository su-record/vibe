 ...
 * 계정 정보 로드
 */
export function loadAccounts(): GeminiStorage | null {
  try {
    ...
    return JSON.parse(content) as GeminiStorage;
  } catch (error) {
    console.error('Account info load failed:', (error as Error).message);
    return null;
  }
}
 ...
 * 계정 정보 저장
 */
export function saveAccounts(storage: GeminiStorage): void {
  try {
    const storagePath = getStoragePath();
    ensureDir(path.dirname(storagePath));
    fs.writeFileSync(storagePath, JSON.stringify(storage, null, 2), 'utf-8');
  } catch (error) {
    console.error('Account info save failed:', (error as Error).message);
    throw error;
  }
}
 ...
 * 새 계정 추가
 */
export function addAccount(account: GeminiAccount): GeminiStorage {
 ...
 * 모든 계정 목록 가져오기
 */
export function getAllAccounts(): GeminiAccount[] {
  const storage = loadAccounts();
...
export function clearAccounts(): void {
  try {
    ...
    console.error('Account info deletion failed:', (error as Error).message);
  }
}
 ...
 * 토큰 만료 여부 확인
 */
export function isTokenExpired(account: GeminiAccount | null): boolean {
 ...
 * 액세스 토큰 업데이트
 */
export function updateAccessToken(email: string, accessToken: string, expires: number): boolean {
  const storage = loadAccounts();
  if (!storage || !storage.accounts) {
    return false;
  }
