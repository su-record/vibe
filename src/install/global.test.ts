...
describe('global surfaces — one copy per client home', () => {
  ...
  it('the dotted skill directories of ≤ 4.1.7 are swept by the next command and by uninstall; a project skill next to them stays', () => {
    setupGlobal(home);
    const skills = path.join(home, '.claude', 'skills');
    for (const name of ['vibe.build', 'vibe.prove', 'my-skill']) fs.mkdirSync(path.join(skills, name), { recursive: true });
