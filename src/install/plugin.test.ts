...
import { installPlugin, MARKETPLACE_NAME, pluginPaths, pluginStatus, sweepLegacyPluginStore } from './plugin.js';
...
describe('vibe plugin install / status', () => {
  it('assembles manifest, one entry skill and hooks, and registers the tree in the personal marketplace', () => {
    ...
    expect(fs.readdirSync(path.join(paths.tree, 'skills')).sort()).toEqual(['vibe']);
    ...
    expect(status).toMatchObject({ exists: true, skills: 1, hooks: true, registered: true, manifestVersion: pkg.version });
  });
  ...
  it('reports drift when the tree is stale or missing', () => {
    ...
    fs.rmSync(path.join(paths.tree, 'skills', 'vibe'), { recursive: true });
    const status = pluginStatus(home);
    expect(status.drift).toEqual([`manifest 0.0.1 ≠ package ${status.packageVersion}`, 'skills 0 ≠ 1']);
  });
  ...
  it('keeps authoring guidance internal and advertises no reviewer agents', () => {
    installPlugin(home);
    const tree = pluginPaths(home).tree;
    expect(fs.readdirSync(path.join(tree, 'skills'))).toEqual(['vibe']);
    expect(fs.existsSync(path.join(tree, 'agents'))).toBe(false);
    expect(fs.existsSync(path.join(packageRoot(), 'internal', 'guides', 'code.md'))).toBe(true);
  });
});
