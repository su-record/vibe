...
import { packStages, type Stage } from '../core/checks/packs.js';
...
/** Codex reads custom agents as TOML (name · description · developer_instructions); one per reviewer stage. */
export function codexAgentToml(pack: string, stage: Stage): string | null {
  if (!fs.existsSync(stage.file)) return null;
  const body = fs.readFileSync(stage.file, 'utf-8').trim();
  const role = `stage ${stage.order} ${stage.name.replace(/-/g, ' ')}`;
  const name = agentName(pack, stage);
  return `# Generated by vibe from reviewers/${pack}/${stage.order}-${stage.name}.md — the ${role} of the antislop-${pack} pack.\nname = "${name}"\ndescription = "antislop-${pack} ${role}: judges the submitted artifact and returns only PASS or a REJECT list; never rewrites."\ndeveloper_instructions = ${tomlString(body)}\n`;
}

/** The agent name a client sees: the text packs keep `ko-copy-editor`, other packs read `design-art-director`. */
export function agentName(pack: string, stage: Stage): string {
  return `${pack}-${stage.name}`;
}
...
export function assemblePlugin(tree: string): string[] {
  ...
  for (const skill of languagePacks()) {
    const pack = skill.replace('antislop-', '');
    for (const stage of packStages(pack)) {
      const toml = codexAgentToml(pack, stage);
      if (!toml) continue;
      const name = `${agentName(pack, stage)}.toml`;
      writeAtomic(path.join(tree, 'agents', name), toml);
      written.push(`agents/${name}`);
    }
  }
