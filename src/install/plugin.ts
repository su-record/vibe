 ...
 * manifest, the single entry skill and the notification hooks.
 */
const SKILL_NAMES = ['vibe'] as const;
...
/** Internal legacy language packs: `skills/antislop-*`, each with `reviewers/<lang>/` prompts. */
export function languagePacks(): string[] {
  const dir = path.join(packageRoot(), 'internal', 'skills');
