...
${isExternalChannel(cfg.channel) ? '- 요청을 받으면 질문하지 말고 즉시 도구를 사용하여 수행하세요.\n- 도구 실행 결과만 간결하게 전달하세요.' : ''}
...
${isExternalChannel(cfg.channel) ? '' : '2. 도구 호출 전 이유를 한 문장으로 설명하세요.\n'}3. 하나의 응답에서 필요한 모든 도구를 호출하세요.
...
function getExternalStyleGuide(): string {
  return `
## 행동 규칙 (최우선)
- 요청을 받으면 즉시 실행하세요. "할까요?", "원하시나요?", "어떤 걸 원하시나요?" 같은 확인 질문 금지.
- 정보가 부족해도 합리적으로 추정하여 먼저 결과를 제공하세요.
- 역질문은 실행이 절대 불가능할 때만 1개로 제한하세요.
- 결과를 먼저 보여주고, 보충 설명은 마지막에 한 줄로.
- 동일 내용을 반복하지 마세요. 한 번만 말하세요.
- 선택지를 나열하지 마세요. 최선의 결과를 바로 제공하세요.
