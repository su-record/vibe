...
export function checkProcess(command: string, options: { cwd: string; timeoutMs: number; input?: string; args?: string[]; env?: NodeJS.ProcessEnv }) {
  ...
  return observe(child, options.timeoutMs, options.input ?? '');
}

function observe(child: ChildProcessWithoutNullStreams, timeoutMs: number, input: string) {
  ...
  return new Promise<Result>((resolve) => {
    ...
    for (const name of ['stdout', 'stderr'] as const) child[name].on('data', (chunk: Buffer) => { if (!settled && streams.add(name, chunk)) stop('capture-overflow'); });
