...
`;
  ...
  private static async ensureScriptExists(): Promise<string> {
    ...
    this.scriptPath = path.join(os.tmpdir(), `core-parser-${process.pid}.py`);
   ...
   */
  private static async executePython(code: string, action: 'symbols' | 'complexity'): Promise<Record<string, unknown>> {
    ...
    try {
      ...
      codePath = path.join(os.tmpdir(), `core-code-${Date.now()}-${Math.random().toString(36).substr(2, 9)}.py`);
  ...
  public static async findSymbols(code: string): Promise<PythonSymbol[]> {
    ...
    return (result.symbols as PythonSymbol[]) || [];
  }
  ...
  public static async analyzeComplexity(code: string): Promise<PythonComplexity> {
    ...
    return {
      cyclomaticComplexity: (result.cyclomaticComplexity as number) || 1,
      functions: (result.functions as PythonComplexity['functions']) || [],
      classes: (result.classes as PythonComplexity['classes']) || []
    };
  }
