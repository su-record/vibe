...
export class MemorySearch {
  ...
  private searchFulltext(query: string, limit: number, category?: string): MemoryItem[] {
    ...
    if (category) {
      const stmt = this.db.prepare(`
        ...
        LIMIT ?
      `);
      return stmt.all(MemoryStorage.sanitizeFTS5Query(query), category, limit) as MemoryItem[];
    }
   ...
   */
  private getMemoryPriorities(keys: string[]): Map<string, number> {
    ...
    try {
      ...
      const rows = this.db.prepare(
        `SELECT key, priority FROM memories WHERE key IN (${placeholders})`,
      ).all(...keys) as Array<{ key: string; priority: number }>;
