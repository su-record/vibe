...
import { getGptOAuthRefreshToken } from '../config/GlobalConfigManager.js';
...
/**
 * OAuth refresh token 가져오기 (config.json 우선 → 환경변수 fallback)
 */
function getRefreshTokenFromEnv(): string | null {
  return getGptOAuthRefreshToken() || process.env.GPT_OAUTH_REFRESH_TOKEN || null;
}

// 메모리 캐시 (프로세스 수명 동안 유효)
let cachedAccessToken: string | null = null;
let cachedExpires = 0;
let cachedEmail: string | null = null;
let cachedAccountId: string | null = null;
let cachedPlan: ChatGptPlan = 'free';

/**
 * 유효한 액세스 토큰 가져오기 (환경변수 기반, 필요시 갱신)
 */
export async function getValidAccessToken(): Promise<ValidAccessToken> {
  ...
  if (!refreshToken) {
    throw new Error(
      'GPT OAuth not configured. Set GPT_OAUTH_REFRESH_TOKEN env variable, or run vibe gpt auth.'
    );
  }

  // 캐시된 토큰이 유효하면 재사용
  const now = Date.now();
  if (cachedAccessToken && cachedExpires > now + 5 * 60 * 1000) {
    return {
  ...
  // 토큰 갱신 (TokenRefresher로 race condition 방지)
  const result = await tokenRefresher.refreshWithLock(
    ...
    async () => {
      ...
      cachedExpires = newTokens.expires;
      cachedEmail = extractEmailFromToken(newTokens.accessToken);
      cachedAccountId = extractAccountId(newTokens.accessToken);
      cachedPlan = extractPlanFromToken(newTokens.accessToken);
      return {
    ...
    () => {
      if (cachedAccessToken && cachedExpires > Date.now()) {
        return { accessToken: cachedAccessToken, expires: cachedExpires };
      }
      return null;
    }
  );
  ...
  return {
    ...
    email: cachedEmail || extractEmailFromToken(result.accessToken) || 'unknown',
    accountId: cachedAccountId || extractAccountId(result.accessToken) || undefined,
    plan: cachedPlan,
  };
}
