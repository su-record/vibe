 ...
 */
export function scopeRegistryOverride(raw: string | null | undefined): string | null {
  const v = raw?.trim();
...
function detectScopeRegistryOverride(): string | null {
  try {
 ...
 */
export function registryLatest(): string | null {
  try {
    const v = execSync(
      `npm view @su-record/vibe version --@su-record:registry=${PUBLISH_REGISTRY}`,
 ...
 */
export function formatVersionParity(
  installed: string,
  latest: string | null,
  scopeOverride: string | null = null,
): string {
  if (latest === null || installed === latest) return '';

  const required = (getPackageJson() as { engines?: { node?: string } }).engines?.node;
  const lines = [
  ...
  if (scopeOverride) {
    lines.push(
      `    @su-record 스코프가 ${scopeOverride} 로 지정돼 있습니다 (.npmrc)`,
      '    이 설정은 --registry 보다 우선합니다 — 그 레지스트리가 뒤처진 만큼 upgrade 도 뒤처집니다',
      '    조치: npm config delete @su-record:registry  (또는 해당 .npmrc 줄 제거)',
    );
  }
  if (required) {
  ...
  lines.push(
    '    확인: which -a vibe  ·  npm prefix -g  ·  npm root -g',
    '    PATH 의 vibe 와 npm prefix 가 다르면 설치는 성공해도 다른 곳에 깔린다',
  );
  return lines.join('\n') + '\n';
}
