...
export async function checkCouplingCohesion(args: { code: string; type?: string; checkDependencies?: boolean }): Promise<ToolResult> {
  ...
  interface CouplingResults {
    ...
    uniqueFunctionCalls: number;
    threshold: number;
    status: string;
    fanOut: number;
    fanOutStatus: string;
  }
  interface CohesionResults {
    type: string;
    score: number;
    level: string;
    status: string;
  ...
  interface CouplingAnalysisResults {
    ast?: AstResults;
  ...
  const initialCoupling: CouplingResults = {
  ...
  const initialCohesion: CohesionResults = { type: couplingType, score: 0, level: 'low', status: 'pending' };

  const couplingAnalysis = {
    action: 'check_coupling_cohesion',
    type: couplingType,
    checkDependencies,
    results: { coupling: initialCoupling, cohesion: initialCohesion } as CouplingAnalysisResults,
    overallScore: 0,
    issues: [] as string[],
