...
export interface BenchRun {
  ...
  cost?: { measured?: boolean; truncated?: boolean; toolCalls?: number | null };
}
...
function range(values: number[]): BenchRange | null {
  if (values.length === 0) return null;
  const sum = values.reduce((acc, v) => acc + v, 0);
  return { min: Math.min(...values), max: Math.max(...values), mean: sum / values.length };
}

function exclusionOf(run: BenchRun): BenchExclusion | null {
  const cost = run.cost;
  if (!cost || cost.measured !== true) return 'cost-unmeasured';
  if (cost.truncated === true) return 'cost-truncated';
  return null;
}
 ...
 */
export function summarizeArm(armId: string, runs: readonly BenchRun[]): ArmSummary {
  const mine = runs.filter((run) => run.armId === armId);
  const excluded: Record<BenchExclusion, number> = { 'cost-unmeasured': 0, 'cost-truncated': 0 };
  const usable: BenchRun[] = [];
...
function taskSetsDiffer(a: ArmSummary, b: ArmSummary): boolean {
  if (a.taskSetHashes.length !== 1 || b.taskSetHashes.length !== 1) return true;
  return a.taskSetHashes[0] !== b.taskSetHashes[0];
}

function overlaps(a: BenchRange, b: BenchRange): boolean {
...
export function compareArms(
  a: ArmSummary,
  b: ArmSummary,
  metric: BenchMetric = 'iterations',
  minRunsPerArm: number = DEFAULT_MIN_RUNS_PER_ARM,
): BenchComparison {
  const base = { metric, arms: [a, b] as [ArmSummary, ArmSummary], delta: null };
