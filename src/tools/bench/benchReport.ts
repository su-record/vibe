...
import type { ArmSummary, BenchComparison, BenchRange } from './benchCompare.js';

function formatRange(label: string, value: BenchRange | null): string {
  if (!value) return `- ${label}: 관측값 없음`;
  return `- ${label}: ${value.min}~${value.max} (평균 ${value.mean.toFixed(2)})`;
}

function formatExclusions(arm: ArmSummary): string[] {
...
function formatArm(arm: ArmSummary): string[] {
  return [
    '',
    `### ${arm.armId}`,
    '',
...
const VERDICT_LABEL: Record<BenchComparison['verdict'], string> = {
...
export function formatBenchReport(input: BenchReportInput): string {
