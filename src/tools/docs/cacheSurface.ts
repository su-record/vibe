...
export const REQUIRED_SUBSECTIONS = ['Model Experience', 'KV Cache effect'] as const;
...
function hasSubsection(body: string, label: string): boolean {
...
function tableEntries(body: string): string[] {
  const out: string[] = [];
  for (const line of body.split('\n')) {
    if (!line.trimStart().startsWith('|')) continue;
    for (const m of line.matchAll(/`([^`\n]+)`/g)) {
      const v = m[1].trim();
      if (/\.(md|js|ts|json)$/.test(v)) out.push(v);
    }
  }
  return out;
}
...
/** 문서를 절 단위로 쪼갠다 */
export function parseSurfaceDoc(content: string): SurfaceSection[] {
...
/** 모든 절이 두 질문에 답했는가 */
export function checkRequiredSubsections(sections: SurfaceSection[]): SurfaceFinding[] {
 ...
 */
export function checkEnumeratedSurfaces(
...
export function checkCacheSurfaceDoc(
  content: string,
  actual: Record<string, readonly string[]>,
): SurfaceFinding[] {
