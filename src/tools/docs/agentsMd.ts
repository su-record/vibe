...
export interface AgentsMdRules {
  ...
  protect: string[];
  substitutions: { find: string; replace: string; why: string }[];
  /** 두 하네스가 의도적으로 다른 문단. 치환 이후 텍스트에 적용된다 */
  overrides: { find: string; replace: string; why: string }[];
}
...
export function generateAgentsMd(source: string, rules: AgentsMdRules): GenerateResult {
  ...
  for (const s of rules.substitutions) text = splitAll(text, s.find, s.replace);
  ...
  for (const o of rules.overrides) {
    ...
    if (hits !== 1) {
      findings.push(
        `override 가 ${hits}번 매치됐다 (정확히 1번이어야 한다): ${JSON.stringify(o.find.slice(0, 60))}`,
      );
      continue;
    }
