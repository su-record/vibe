...
description: "Design system generator. Detect industry category and derive style/color/typography strategy, Generate and persist the MASTER.md design system (tokens, palette, type, spacing), Advise on page layout, data-visualization, and framework-specific implementation. 산업 분석 → MASTER.md(CSS 변수/팔레트/타이포/스페이싱) + 레이아웃/차트 가이드 생성·저장."
model: inherit
