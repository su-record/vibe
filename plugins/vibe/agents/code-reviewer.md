---
name: code-reviewer
description: "Parameterized code reviewer — caller passes focus: correctness, architecture, performance, complexity, data-integrity, test-coverage, idioms, git-history. Review changed code along caller-specified focus dimensions, Surface P1/P2/P3 findings with concrete locations and fixes, Cover correctness, architecture, performance, complexity, data integrity, test coverage, and language idioms. Invoked by /vibe.review with a focus (not a standalone auto-trigger)."
model: inherit
