 ...
 */
export async function coreZaiOrchestrate(prompt, systemPrompt, options = {}) {
 ...
 */
export async function coreZaiUiImplement(prompt, systemPrompt = 'You are a senior UI engineer. Produce production-quality, accessible UI code that matches the project stack and design system.') {
    const start = Date.now();
    const result = await chat({
        model: ZAI_TOP_MODEL,
        plan: 'coding',
