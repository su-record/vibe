 ...
 */
import { getZaiApiKey, getZaiCodingApiKey, getModelOverride, } from '../config/GlobalConfigManager.js';
...
export function resolveApiKey(plan) {
    if (plan === 'coding')
        return getZaiCodingApiKey() ?? process.env.ZAI_CODING_API_KEY ?? null;
    return getZaiApiKey() ?? process.env.ZAI_API_KEY ?? null;
}
export function baseUrlFor(plan) {
...
function pickPlan(requested) {
    if (requested)
        return requested;
    return resolveApiKey('coding') ? 'coding' : 'general';
}
function resolveModel(plan, explicit) {
    if (explicit)
        return explicit;
...
export async function fetchAvailableModels(plan = 'general') {
    ...
    try {
        ...
        if (!res.ok)
            return curated;
        const data = (await res.json());
        const ids = (data.data ?? []).map((m) => m.id).filter((id) => Boolean(id));
        if (!ids.length)
            return curated;
