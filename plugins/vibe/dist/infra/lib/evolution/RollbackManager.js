...
export class RollbackManager {
    ...
    disable(generationId) {
        ...
        if (gen.status === 'disabled' || gen.status === 'deleted')
            return;
    ...
    rollback(generationId) {
        ...
        if (!gen)
            throw new Error(`Generation not found: ${generationId}`);
        if (!gen.parentId)
            throw new Error(`No parent version for: ${generationId}`);
        const parent = this.registry.getById(gen.parentId);
        if (!parent)
            throw new Error(`Parent generation not found: ${gen.parentId}`);
        const transaction = this.db.transaction(() => {
