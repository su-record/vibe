...
export class DescriptionOptimizer {
     ...
     */
    generateTriggerEvalSet(skillName, description, existingSkillDescriptions) {
        const descWords = this.extractKeywords(description);
        ...
        if (existingSkillDescriptions) {
            ...
            for (const [otherName, otherDesc] of existingSkillDescriptions) {
                if (otherName === skillName || nearMissCount >= 5)
                    break;
     ...
     */
    validateTriggers(skillName, description, queries, existingSkillDescriptions) {
        const evalQueries = queries ?? this.generateTriggerEvalSet(skillName, description, existingSkillDescriptions);
     ...
     */
    checkCollisions(skillName, description, existingSkills) {
        const descWords = this.extractKeywords(description);
        const collisions = [];
     ...
     */
    batchValidate(skills, accuracyThreshold = 0.7) {
        const results = [];
