...
export class InsightExtractor {
     ...
     */
    extractFromRecent(limit = 50) {
        const result = { newInsights: [], mergedInsights: [], skippedCount: 0, errorCount: 0 };
        // Collect recent data
    ...
    processReflectionTopics(refl, topicCounts) {
    ...
    processObservationTopics(obs, topicCounts) {
    ...
    classifyTopic(topic, descriptions) {
    ...
    extractTags(topic, descriptions) {
        const tags = new Set();
        const text = (topic + ' ' + descriptions.join(' ')).toLowerCase();
