// Advanced memory search operations
// Extracted from MemoryManager for better separation of concerns
import { MemoryStorage } from './MemoryStorage.js';
export class MemorySearch {
    db;
    storage;
    graph;
    constructor(storage, graph) {
        this.storage = storage;
        this.graph = graph;
        this.db = storage.getDatabase();
    }
    /**
     * Advanced search with multiple strategies (sync — backward compatible)
     * vector/hybrid 전략은 context_aware로 fallback (동기)
     */
    searchAdvanced(query, strategy, options = {}) {
        const { limit = 20, category, startKey, depth = 2 } = options;
        switch (strategy) {
            case 'keyword':
                return this.searchKeyword(query, limit, category);
            case 'fulltext':
                return this.searchFulltext(query, limit, category);
            case 'graph_traversal':
                if (!startKey)
                    return this.searchKeyword(query, limit, category);
                return this.graph.getRelatedMemories(startKey, depth);
            case 'temporal':
                return this.searchTemporal(query, limit);
            case 'priority':
                return this.searchByPriority(query, limit);
            case 'context_aware':
                return this.searchContextAware(query, limit, category);
            case 'vector':
            case 'hybrid':
                // 동기 호출 시 context_aware fallback. 비동기는 searchAdvancedAsync 사용.
                return this.searchContextAware(query, limit, category);
            default:
                return this.storage.search(query);
        }
    }
    /**
     * Advanced search with vector support (async)
     * vector/hybrid 전략에서 실제 벡터 검색 수행
     */
    async searchAdvancedAsync(query, strategy, options = {}) {
        const { limit = 20, category } = options;
        switch (strategy) {
            case 'vector':
                return this.searchVector(query, limit);
            case 'hybrid':
                return this.searchHybrid(query, limit, category);
            default:
                return this.searchAdvanced(query, strategy, options);
        }
    }
    searchKeyword(query, limit, category) {
        return this.searchKeywordLike(query, limit, category);
    }
    searchFulltext(query, limit, category) {
        if (!this.storage.isFTS5Available()) {
            return this.searchKeywordLike(query, limit, category);
        }
        if (category) {
            const stmt = this.db.prepare(`
        SELECT m.*, bm25(memories_fts) as rank
        FROM memories_fts fts
        JOIN memories m ON m.rowid = fts.rowid
        WHERE memories_fts MATCH ? AND m.category = ?
        ORDER BY rank
        LIMIT ?
      `);
            return stmt.all(MemoryStorage.sanitizeFTS5Query(query), category, limit);
        }
        return this.storage.searchFTS(query, limit);
    }
    searchKeywordLike(query, limit, category) {
        let sql = `
      SELECT * FROM memories
      WHERE (key LIKE ? OR value LIKE ?)
    `;
        const params = [`%${query}%`, `%${query}%`];
        if (category) {
            sql += ` AND category = ?`;
            params.push(category);
        }
        sql += ` ORDER BY priority DESC, timestamp DESC LIMIT ?`;
        params.push(limit);
        return this.db.prepare(sql).all(...params);
    }
    searchTemporal(query, limit) {
        const sql = `
      SELECT * FROM memories
      WHERE key LIKE ? OR value LIKE ?
      ORDER BY timestamp DESC
      LIMIT ?
    `;
        return this.db.prepare(sql).all(`%${query}%`, `%${query}%`, limit);
    }
    searchByPriority(query, limit) {
        const sql = `
      SELECT * FROM memories
      WHERE key LIKE ? OR value LIKE ?
      ORDER BY priority DESC, lastAccessed DESC
      LIMIT ?
    `;
        return this.db.prepare(sql).all(`%${query}%`, `%${query}%`, limit);
    }
    searchContextAware(query, limit, category) {
        // Use FTS5 rank if available for better relevance scoring
        if (this.storage.isFTS5Available()) {
            try {
                let sql;
                const params = [];
                if (category) {
                    sql = `
            SELECT m.*,
              (bm25(memories_fts) * -1 + m.priority * 0.5) as relevance_score
            FROM memories_fts fts
            JOIN memories m ON m.rowid = fts.rowid
            WHERE memories_fts MATCH ? AND m.category = ?
            ORDER BY relevance_score DESC, m.lastAccessed DESC
            LIMIT ?
          `;
                    params.push(MemoryStorage.sanitizeFTS5Query(query), category, limit);
                }
                else {
                    sql = `
            SELECT m.*,
              (bm25(memories_fts) * -1 + m.priority * 0.5) as relevance_score
            FROM memories_fts fts
            JOIN memories m ON m.rowid = fts.rowid
            WHERE memories_fts MATCH ?
            ORDER BY relevance_score DESC, m.lastAccessed DESC
            LIMIT ?
          `;
                    params.push(MemoryStorage.sanitizeFTS5Query(query), limit);
                }
                const results = this.db.prepare(sql).all(...params);
                if (results.length > 0)
                    return results;
            }
            catch {
                // FTS5 query failed, fall through to LIKE-based scoring
            }
        }
        // LIKE-based fallback
        let sql = `
      SELECT *,
        (CASE WHEN key LIKE ? THEN 3 ELSE 0 END +
         CASE WHEN value LIKE ? THEN 2 ELSE 0 END +
         priority * 0.5) as relevance_score
      FROM memories
      WHERE key LIKE ? OR value LIKE ?
    `;
        const params = [`%${query}%`, `%${query}%`, `%${query}%`, `%${query}%`];
        if (category) {
            sql += ` AND category = ?`;
            params.push(category);
        }
        sql += ` ORDER BY relevance_score DESC, lastAccessed DESC LIMIT ?`;
        params.push(limit);
        return this.db.prepare(sql).all(...params);
    }
    // ==========================================================================
    // Vector & Hybrid search
    // ==========================================================================
    /**
     * 순수 벡터 유사도 검색
     * 벡터 불가 시 BM25 fallback
     */
    async searchVector(query, limit) {
        const vectorStore = this.storage.getVectorStore();
        const provider = this.storage.getEmbeddingProvider();
        if (!vectorStore || !provider) {
            return this.storage.search(query);
        }
        try {
            const result = await provider.embed([query]);
            if (result.embeddings.length === 0) {
                return this.storage.search(query);
            }
            const vectorResults = vectorStore.searchMemoryVectors(result.embeddings[0], limit);
            return this.resolveMemoryItems(vectorResults.map(v => v.key));
        }
        catch {
            return this.storage.search(query);
        }
    }
    /**
     * 하이브리드 검색: Vector + BM25 + Priority
     * 벡터 불가 시 context_aware fallback
     */
    async searchHybrid(query, limit, category) {
        const vectorStore = this.storage.getVectorStore();
        const provider = this.storage.getEmbeddingProvider();
        if (!vectorStore || !provider) {
            return this.searchContextAware(query, limit, category);
        }
        try {
            // 1. 벡터 검색
            const embResult = await provider.embed([query]);
            if (embResult.embeddings.length === 0) {
                return this.searchContextAware(query, limit, category);
            }
            const vectorResults = vectorStore.searchMemoryVectors(embResult.embeddings[0], limit * 3);
            const vectorScores = new Map();
            for (const v of vectorResults) {
                vectorScores.set(v.key, v.similarity);
            }
            // 2. BM25 검색
            const bm25Results = this.getBM25MemoryScores(query, limit * 3, category);
            // 3. 합집합 키 수집 후 배치로 priority 조회
            const allKeys = new Set([
                ...vectorScores.keys(),
                ...bm25Results.keys(),
            ]);
            const priorityMap = this.getMemoryPriorities([...allKeys]);
            // 4. 하이브리드 스코어 계산
            const scored = [];
            for (const key of allKeys) {
                const vecScore = vectorScores.get(key) ?? 0;
                const bm25Score = bm25Results.get(key) ?? 0;
                const priorityScore = priorityMap.get(key) ?? 0;
                const score = vecScore * 0.5 +
                    bm25Score * 0.3 +
                    priorityScore * 0.2;
                scored.push({ key, score });
            }
            // 5. 정렬 + limit
            scored.sort((a, b) => b.score - a.score);
            const topKeys = scored.slice(0, limit).map(s => s.key);
            return this.resolveMemoryItems(topKeys);
        }
        catch {
            return this.searchContextAware(query, limit, category);
        }
    }
    /**
     * BM25 스코어 맵 생성 (정규화: 0~1)
     */
    getBM25MemoryScores(query, limit, category) {
        const scores = new Map();
        if (!this.storage.isFTS5Available())
            return scores;
        try {
            let sql;
            const params = [];
            if (category) {
                sql = `
          SELECT m.key, bm25(memories_fts) as rank
          FROM memories_fts fts
          JOIN memories m ON m.rowid = fts.rowid
          WHERE memories_fts MATCH ? AND m.category = ?
          ORDER BY rank
          LIMIT ?
        `;
                params.push(MemoryStorage.sanitizeFTS5Query(query), category, limit);
            }
            else {
                sql = `
          SELECT m.key, bm25(memories_fts) as rank
          FROM memories_fts fts
          JOIN memories m ON m.rowid = fts.rowid
          WHERE memories_fts MATCH ?
          ORDER BY rank
          LIMIT ?
        `;
                params.push(MemoryStorage.sanitizeFTS5Query(query), limit);
            }
            const rows = this.db.prepare(sql).all(...params);
            for (const row of rows) {
                // BM25: negative (more negative = better). Normalize to 0~1.
                const clamped = Math.max(row.rank, -20);
                scores.set(row.key, Math.abs(clamped) / 20);
            }
        }
        catch {
            // FTS5 query failed
        }
        return scores;
    }
    /**
     * 메모리 priority 정규화 (0~1)
     */
    getMemoryPriority(key) {
        try {
            const row = this.db.prepare('SELECT priority FROM memories WHERE key = ?').get(key);
            if (!row)
                return 0;
            return Math.min(row.priority / 2, 1);
        }
        catch {
            return 0;
        }
    }
    /**
     * 여러 키의 priority를 한 번의 쿼리로 일괄 조회 (N+1 방지)
     */
    getMemoryPriorities(keys) {
        const result = new Map();
        if (keys.length === 0)
            return result;
        try {
            const placeholders = keys.map(() => '?').join(',');
            const rows = this.db.prepare(`SELECT key, priority FROM memories WHERE key IN (${placeholders})`).all(...keys);
            for (const row of rows) {
                result.set(row.key, Math.min(row.priority / 2, 1));
            }
        }
        catch {
            // fallback: return empty map (scores default to 0)
        }
        return result;
    }
    /**
     * 키 배열로 MemoryItem[] 조회 (순서 유지)
     */
    resolveMemoryItems(keys) {
        if (keys.length === 0)
            return [];
        const placeholders = keys.map(() => '?').join(',');
        const rows = this.db.prepare(`SELECT * FROM memories WHERE key IN (${placeholders})`).all(...keys);
        const rowMap = new Map(rows.map(r => [r.key, r]));
        return keys
            .map(k => rowMap.get(k))
            .filter((item) => item !== undefined);
    }
}
//# sourceMappingURL=MemorySearch.js.map