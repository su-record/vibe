...
export class SessionRAGRetriever {
     ...
     * 비동기 retrieve (벡터 포함)
     */
    async retrieveWithVector(options) {
        const startTime = Date.now();
        const { query, limit = DEFAULT_LIMIT, vectorWeight = DEFAULT_VECTOR_WEIGHT, } = options;
     ...
     * 벡터 스코어 맵 생성
     */
    getVectorScores(queryEmbedding) {
    ...
    // ==========================================================================
    scoreDecisions(query, limit, weights, vectorScores, taskContext) {
    ...
    scoreConstraints(query, limit, weights, vectorScores, taskContext) {
    ...
    scoreGoals(query, limit, weights, vectorScores, taskContext) {
    ...
    scoreEvidence(query, limit, weights, vectorScores, taskContext) {
    ...
    // ==========================================================================
    getCandidateDecisions(_query, bm25Scores, limit, taskContext) {
        ...
        if (taskContext) {
            ...
            if (sf.clause) {
                const rows = this.db.prepare(`SELECT * FROM session_decisions WHERE 1=1${sf.clause} ORDER BY priority DESC, timestamp DESC LIMIT ?`).all(...sf.params, limit);
                return rows;
            }
        }
        return this.store.listDecisions(undefined, undefined, limit);
    }
    getCandidateConstraints(_query, bm25Scores, limit, taskContext) {
        if (bm25Scores.size > 0) {
            const ids = [...bm25Scores.keys()].slice(0, limit);
            return ids
        ...
        if (taskContext) {
            const sf = TaskContext.buildScopeFilter(taskContext);
            if (sf.clause) {
                const rows = this.db.prepare(`SELECT * FROM session_constraints WHERE 1=1${sf.clause} ORDER BY timestamp DESC LIMIT ?`).all(...sf.params, limit);
                return rows;
            }
        }
        return this.store.listConstraints(undefined, undefined, undefined, limit);
    }
    getCandidateGoals(_query, bm25Scores, limit, taskContext) {
        if (bm25Scores.size > 0) {
            const ids = [...bm25Scores.keys()].slice(0, limit);
            return ids
        ...
        if (taskContext) {
            const sf = TaskContext.buildScopeFilter(taskContext);
            if (sf.clause) {
                const rows = this.db.prepare(`SELECT * FROM session_goals WHERE 1=1${sf.clause} ORDER BY priority DESC, timestamp DESC LIMIT ?`).all(...sf.params, limit);
                return rows;
            }
        }
        return this.store.listGoals(undefined, undefined, limit);
    }
    getCandidateEvidence(_query, bm25Scores, limit, taskContext) {
        if (bm25Scores.size > 0) {
            const ids = [...bm25Scores.keys()].slice(0, limit);
            return ids
        ...
        if (taskContext) {
            const sf = TaskContext.buildScopeFilter(taskContext);
            if (sf.clause) {
                const rows = this.db.prepare(`SELECT * FROM session_evidence WHERE 1=1${sf.clause} ORDER BY timestamp DESC LIMIT ?`).all(...sf.params, limit);
                return rows;
            }
        }
