...
export class VectorStore {
     ...
     * 저장된 세션 벡터 수
     */
    getSessionVectorCount(entityType) {
        if (entityType) {
            const row = this.db.prepare('SELECT COUNT(*) as cnt FROM session_vectors WHERE entityType = ?').get(entityType);
            return row.cnt;
        }
        const row = this.db.prepare('SELECT COUNT(*) as cnt FROM session_vectors').get();
        return row.cnt;
    }
}
