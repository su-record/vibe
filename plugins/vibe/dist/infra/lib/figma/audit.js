...
function pushFrom(node, trail, findings) {
...
function walkAudit(node, trail, findings, counter) {
    counter.n += 1;
    const nextTrail = [...trail, node.name ?? node.id];
    pushFrom(node, nextTrail, findings);
    if (node.children?.length) {
        for (const child of node.children)
            walkAudit(child, nextTrail, findings, counter);
    }
}
export async function auditNode(fileKey, nodeId, depth) {
    const token = loadToken();
    const depthParam = depth ? `&depth=${depth}` : '';
    const data = await figmaFetch(`/files/${fileKey}/nodes?ids=${nodeId}${depthParam}`, token);
