...
function extractCSS(node) {
    ...
    if (node.type === 'TEXT' && node.style) {
        ...
        if (s.fontSize) {
            ...
            raw.fontSize = s.fontSize;
        }
        if (s.fontWeight) {
            css.fontWeight = String(s.fontWeight);
            raw.fontWeight = s.fontWeight;
        }
        if (s.lineHeightPx) {
            css.lineHeight = `${s.lineHeightPx}px`;
            raw.lineHeightPx = s.lineHeightPx;
        }
        if (s.letterSpacing) {
            css.letterSpacing = `${s.letterSpacing}px`;
            raw.letterSpacing = s.letterSpacing;
        }
        const textAlignMap = {
...
function isDesignTextNode(n) {
    ...
    if (fills.length >= 2)
        return true;
    if (fills.some(f => typeof f.type === 'string' && f.type.startsWith('GRADIENT_')))
        return true;
    if ((n.effects || []).some(e => e.visible !== false && (e.type === 'DROP_SHADOW' || e.type === 'INNER_SHADOW')))
        return true;
    if ((n.strokes || []).some(s => s.visible !== false))
        return true;
    return false;
}
/** Q1: any descendant carries meaningful text content. */
function hasTextDescendantRaw(n) {
    if (n.type === 'TEXT' && typeof n.characters === 'string' && n.characters.trim().length)
        return true;
...
export function walkNode(node) {
    ...
    if (hasTextDescendantRaw(node))
        result.hasTextChildren = true;
    if (hasRepeatingInstancesRaw(node))
        result.hasInstanceRepeat = true;
    if (node.type === 'TEXT' && isDesignTextNode(node))
        result.isDesignText = true;
    const vcnt = vectorChildCountRaw(node);
    if (vcnt > 0)
        result.vectorChildCount = vcnt;
