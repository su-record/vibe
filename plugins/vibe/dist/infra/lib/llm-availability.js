...
function checkAuthExists(configDir, authFileName) {
...
export function detectLlmAvailability() {
    ...
    const antigravity = antigravityInstalled && (checkAuthExists(antigravityDir, 'settings.json') ||
        !!process.env.ANTIGRAVITY_API_KEY);
    // ZAI 는 CLI 가 없다 — coding/general 키 중 하나라도 있으면 활성화
    const zai = Boolean(getZaiCodingApiKey() || getZaiApiKey() ||
...
/** 캐시 초기화 (테스트용) */
export function resetLlmAvailabilityCache() {
    cached = null;
}
//# sourceMappingURL=llm-availability.js.map