...
import { HEX_COLOR_REGEX, DEFAULT_COLOR_PRIMARY, DEFAULT_COLOR_SECONDARY, DEFAULT_COLOR_CTA, DEFAULT_COLOR_BACKGROUND, DEFAULT_COLOR_TEXT, DEFAULT_COLOR_BORDER, PRIORITY_SCORE_EXACT, PRIORITY_SCORE_KEYWORD, PRIORITY_SCORE_OTHER, PROJECT_NAME_REGEX, WINDOWS_RESERVED_NAMES, } from './constants.js';
...
export class DesignSystemGenerator {
    ...
    detectCategory(query) {
        ...
        const dashboardLayout = firstItem['Dashboard Style (if applicable)'] || null;
    ...
    applyReasoning(category) {
        const searchResult = this.searchService.search(category, 'ui-reasoning', 1);
        if (searchResult.results.length === 0) {
    ...
    multiDomainSearch(query, stylePriority) {
        ...
        const styleRow = this.selectBestMatch(this.searchService.search(styleQuery, 'style', 5).results, stylePriority);
        const colorRow = this.selectBestMatch(this.searchService.search(query, 'color', 5).results, [query]);
        const typoRow = this.selectBestMatch(this.searchService.search(query, 'typography', 5).results, [query]);
        const layoutRow = this.selectBestMatch(this.searchService.search(query, 'landing', 5).results, [query]);
        return {
     ...
     */
    selectBestMatch(results, priority) {
        if (results.length === 0) {
            return null;
        }
        let bestScore = -1;
     ...
     */
    buildDesignSystem(params) {
        const { projectName, category, reasoning, multiDomain, dashboardLayout } = params;
        const cssVariables = this.buildCssVariables(multiDomain.colorPalette, multiDomain.typography, multiDomain.style);
        return {
            projectName,
            category,
     ...
     */
    formatPageOverride(ds, pageName, pageQuery) {
        const lines = [];
    ...
    persist(ds, projectName, page) {
        ...
        const baseDir = join(process.cwd(), '.claude', 'vibe', 'design-system', projectName);
     ...
     */
    buildCssVariables(color, typo, style) {
        const vars = {};
        if (color) {
            vars['--color-primary'] = this.validateHexColor(color.primary, DEFAULT_COLOR_PRIMARY);
            vars['--color-secondary'] = this.validateHexColor(color.secondary, DEFAULT_COLOR_SECONDARY);
            vars['--color-cta'] = this.validateHexColor(color.cta, DEFAULT_COLOR_CTA);
            vars['--color-background'] = this.validateHexColor(color.background, DEFAULT_COLOR_BACKGROUND);
            vars['--color-text'] = this.validateHexColor(color.text, DEFAULT_COLOR_TEXT);
            vars['--color-border'] = this.validateHexColor(color.border, DEFAULT_COLOR_BORDER);
        }
        else {
     ...
     */
    parseDesignSystemVariables(raw, vars) {
        const parts = raw
            .split(/[,;\n]+/)
            .map((s) => s.trim())
            .filter(Boolean);
     ...
     */
    hasExactMatch(row, query) {
        const lowerQuery = query.toLowerCase();
        return Object.values(row).some((val) => typeof val === 'string' && val.toLowerCase() === lowerQuery);
    }
    /**
     * Check if row has keyword match with any priority
     */
    hasKeywordMatch(row, priority) {
...
//# sourceMappingURL=DesignSystemGenerator.js.map