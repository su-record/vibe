...
import { DOMAIN_CONFIG, STACK_CONFIG, DOMAIN_KEYWORDS, HOT_CACHE_DOMAINS, MAX_QUERY_LENGTH, MAX_QUERY_TOKENS, DEFAULT_MAX_RESULTS, MAX_RESULTS_UPPER_BOUND, LRU_MAX_SIZE, LRU_TTL_MS, } from './constants.js';
/**
...
export class SearchService {
     ...
     */
    search(query, domain, maxResults = DEFAULT_MAX_RESULTS) {
     ...
     */
    searchStack(query, stack, maxResults = DEFAULT_MAX_RESULTS) {
        const sanitized = this.sanitizeQuery(query);
        const clampedMax = Math.max(1, Math.min(maxResults, MAX_RESULTS_UPPER_BOUND));
    ...
    detectDomain(query) {
        ...
        for (const [domain, keywords] of Object.entries(DOMAIN_KEYWORDS)) {
            ...
            if (matchCount > 0) {
                domainScores.set(domain, matchCount);
            }
        }
        if (domainScores.size === 0) {
            return 'product';
        }
        let maxDomain = 'product';
        let maxScore = 0;
     ...
     */
    ensureDomainLoaded(domain, config) {
     ...
     * Build cache key
     */
    buildCacheKey(type, query, domainOrStack, maxResults) {
