...
/** 데이터 시각화 키워드 */
const DATAVIZ_KEYWORDS = new Set([
 ...
 */
export function isUiUxProject(specContent) {
 ...
 */
export function shouldRunDataViz(specContent) {
    const lowerContent = specContent.toLowerCase();
 ...
 * @returns UI 파일 포함 여부
 */
export function hasUiFileChanges(changedFiles) {
 ...
 */
export function loadDesignSystem(projectName, projectRoot) {
 ...
 */
export function isUiUxAnalysisDisabled(projectRoot) {
    const root = projectRoot ?? process.cwd();
 ...
 */
export function isUiUxDataInstalled() {
    return globalVibeRoots().some(root => existsSync(join(root, 'ui-ux-data', 'version.json')));
}
/**
 ...
 */
export function loadPageOverride(projectName, page, projectRoot) {
...
function projectVibeRoot(projectRoot) {
