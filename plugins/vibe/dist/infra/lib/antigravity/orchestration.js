 ...
 */
export async function coreAntigravityParseSpec(spec) {
 ...
 */
export async function coreAntigravityPlanExecution(task, context) {
    return coreAntigravityOrchestrate(`Task: ${task}\n\nContext:\n${context}`, `You are a Core execution planner. Given a task and context, create a step-by-step execution plan.
Output format: { "steps": [{ "id": 1, "action": "...", "target": "...", "expected": "..." }], "estimatedComplexity": "low|medium|high" }`);
}
/**
 * Core 코드 분석 (빠른 구조 분석)
 */
export async function coreAntigravityAnalyze(code, question) {
    return coreAntigravityOrchestrate(`Code:\n\`\`\`\n${code}\n\`\`\`\n\nQuestion: ${question}`, `You are a code analyzer. Answer the question about the given code concisely.
Output format: { "answer": "...", "confidence": 0.0-1.0, "relatedSymbols": [...] }`);
}
/**
 * Core 다음 액션 결정 (상태 기반)
 */
export async function coreAntigravityDecideNextAction(currentState, availableActions, goal) {
    return coreAntigravityOrchestrate(`Current State:\n${currentState}\n\nAvailable Actions:\n${availableActions.join('\n')}\n\nGoal: ${goal}`, `You are an action decider. Based on the current state and goal, select the best next action.
Output format: { "selectedAction": "...", "reason": "...", "parameters": {} }`);
}
/**
 * Core UI/UX 분석 (검색 없이 내부 지식으로)
 */
export async function coreAntigravityAnalyzeUX(description) {
    return coreAntigravityOrchestrate(description, `You are a UI/UX expert. Analyze the given design description and provide structured feedback.
