...
let _astProject = null;
async function getAstProject() {
    if (!_astProject) {
...
export async function checkCouplingCohesion(args) {
    ...
    // coupling is always assigned unconditionally before use
    // cohesion is assigned in the type branch (class/module/function/component) before use
    const initialCoupling = {
        totalDependencies: 0, externalDependencies: 0, internalDependencies: 0,
        functionCalls: 0, uniqueFunctionCalls: 0, threshold: 0, status: 'pending',
        fanOut: 0, fanOutStatus: 'pending'
    };
    const initialCohesion = { type: couplingType, score: 0, level: 'low', status: 'pending' };
    const couplingAnalysis = {
        action: 'check_coupling_cohesion',
        type: couplingType,
        checkDependencies,
        results: { coupling: initialCoupling, cohesion: initialCohesion },
        overallScore: 0,
        issues: [],
        recommendations: [],
        status: 'pending'
    ...
    try {
        ...
        const requireCalls = sourceFile.getDescendantsOfKind(SyntaxKind.CallExpression).filter((call) => call.getExpression().getText() === 'require');
        // 클래스/함수/모듈 구조 분석
...
//# sourceMappingURL=checkCouplingCohesion.js.map