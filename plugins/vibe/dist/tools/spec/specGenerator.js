...
function buildSpecContent(prd, featureName, phases, options) {
    const { techStack = [], relatedCodePaths = [], designReference, additionalConstraints = [], additionalOutputs = [], contextSources = [], assumptions = [], humanTaste = [], } = options;
...
// Helper Functions
// ============================================
function renderEvidenceRequired(evidence, requirements) {
...
function renderContractTables(requirements, featureName) {
    const targets = requirements.length > 0
        ? requirements
        : [fallbackRequirement(`REQ-${normalizeFileName(featureName)}-001`, 'Build and tests pass for this phase')];
    const requirementRows = targets.map((requirement, index) => `| ${requirement.id} | ${truncateText(requirement.description, 80)} | D${index + 1} |`).join('\n');
    const criterionRows = targets.map((requirement, index) => `| D${index + 1} | ${truncateText(requirement.description, 80)} | Automated verification exits 0 |`).join('\n');
    return `## Requirements
...
function fallbackRequirement(id, description) {
    return { id, description, acceptanceCriteria: [], priority: 'high' };
}
function renderSharedConstraints(additional) {
    return [
