...
export const SPEC_STATUSES = ['DRAFT', 'APPROVED', 'VERIFIED', 'SUPERSEDED', 'REJECTED'];
 ...
 */
export const ANCHOR_REQUIRED_CLASSES = ['feature', 'bug-fix', 'architecture'];
 ...
 */
export function isLifecycleExempt(specPath) {
...
export function parseSpecLifecycle(content) {
...
function checkStatus(header, out) {
    if (!header.status) {
        out.push({
            code: 'no-spec-status',
    ...
    if (!SPEC_STATUSES.includes(header.status)) {
        out.push({
...
function checkClass(header, out) {
    if (!header.specClass) {
        out.push({
            code: 'no-spec-class',
    ...
    if (!SPEC_CLASSES.includes(header.specClass)) {
        out.push({
...
export function anchorsRequired(header) {
    return header.status === 'VERIFIED'
        && ANCHOR_REQUIRED_CLASSES.includes(header.specClass ?? '');
}
function checkAnchors(header, out) {
    if (!anchorsRequired(header))
        return;
 ...
 */
export function checkSpecLifecycle(content, specPath) {
    if (specPath && isLifecycleExempt(specPath))
        return [];
...
//# sourceMappingURL=specLifecycle.js.map