 ...
 * 질문 ID 생성
 */
export function generateQuestionId(category, index) {
    return `Q-${category.toUpperCase()}-${String(index).padStart(3, '0')}`;
}
/**
 * 템플릿에서 질문 생성
 */
export function createQuestionFromTemplate(templateKey, overrides) {
 ...
 * 카테고리별 필수 질문 세트 생성
 */
export function createRequiredQuestionSet(categories) {
    const questions = [];
 ...
 * 질문을 마크다운 형식으로 포맷
 */
export function formatQuestionAsMarkdown(question) {
    const lines = [];
    // Category & ID
    ...
    if (question.options && question.options.length > 0) {
        const selectHint = question.type === 'multi_select' ? '(복수 선택 가능)' : '(하나만 선택)';
        ...
        if (validation) {
            ...
            if (validation.min !== undefined)
                range.push(`최소: ${validation.min}`);
            if (validation.max !== undefined)
                range.push(`최대: ${validation.max}`);
            if (range.length > 0)
                lines.push(`_${range.join(', ')}_`);
        }
    }
    lines.push('');
 ...
 * 전체 질문 세트를 포맷
 */
export function formatQuestionsForUser(input) {
    const lines = [];
    // Header
    lines.push('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    lines.push('📋 요구사항 확인');
 ...
 * 사용자 응답 파싱
 */
export function parseUserResponse(question, rawResponse) {
...
export const askUserQuick = {
     ...
     * 로그인 기능 관련 질문
     */
    login: (featureName = 'login') => askUser({
        featureName,
        categories: ['authentication', 'security', 'session'],
    }),
    /**
     ...
     * 사용자 관리 기능 관련 질문
     */
    userManagement: (featureName = 'user-management') => askUser({
        featureName,
