...
export async function getSessionContext(args) {
    try {
        const { projectName, category, memoryLimit = 15, includeGraph = true, includeTimeline = true, timeRange = '7d', projectPath } = args;
        ...
        if (projectName) {
            memories = memories.filter(m => m.key.toLowerCase().includes(projectName.toLowerCase()) ||
...
function getRelationArrow(relationType) {
    const arrows = {
        'related_to': '↔',
        'depends_on': '←',
        'implements': '→',
