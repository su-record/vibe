...
export const retrieveSessionContextDefinition = {
    ...
    try {
        ...
        if (result.goals.length > 0) {
            ...
            result.goals.forEach(({ item, score, breakdown }) => {
                ...
                markdown += `**Date:** ${new Date(item.timestamp).toLocaleString()}\n`;
                if (item.completedAt)
                    markdown += `**Completed:** ${new Date(item.completedAt).toLocaleString()}\n`;
                markdown += `\n---\n\n`;
            });
        }
        // Evidence
...
//# sourceMappingURL=retrieveSessionContext.js.map