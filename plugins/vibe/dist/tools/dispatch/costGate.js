...
function describe(op) {
    ...
    if (op.kind === 'agent-fanout')
        return `에이전트 ${op.agentCount ?? 0}개 병렬 실행`;
 ...
 */
export function evaluateCostGate(op, options = {}) {
...
/** 스킬이 그대로 출력하는 한 줄 */
export function formatCostGate(op, decision) {
    const what = describe(op);
    if (decision.action === 'proceed')
        return `✅ cost gate 통과 — ${what} (${decision.reason})`;
    if (decision.action === 'record')
        return `📝 cost gate 기록 (autonomous, 묻지 않음) — ${what}: ${decision.reason}`;
    return `⏸️ 승인 필요 — ${what}\n   ${decision.reason}`;
}
//# sourceMappingURL=costGate.js.map