...
export function selectPreviousSemanticTag(tags, currentTag) {
    const current = parseTag(currentTag);
    if (!current)
        throw new Error(`Invalid release tag: ${currentTag}`);
    const candidates = tags
        .map((tag) => ({ tag, version: parseTag(tag) }))
        .filter((item) => item.version !== undefined && compareVersions(item.version, current) < 0)
        .sort((left, right) => compareVersions(right.version, left.version));
    if (!candidates[0])
        throw new Error(`No previous semantic tag found for ${currentTag}`);
 ...
 */
export function selectChangedSpecPaths(nameStatus) {
    const paths = [];
    for (const line of nameStatus.split('\n')) {
        ...
        if (!target || !target.endsWith('.md'))
            continue;
        if (!target.startsWith(SPEC_PATH_PREFIX))
            continue;
        if (status.startsWith('D'))
            continue;
        if (isRenameLike && /^[RC]100$/.test(status))
            continue;
        paths.push(target);
    }
    return paths;
...
export function classifyCommits(commits) {
    return commits.reduce((sections, commit) => {
        if (commit.subject.startsWith('Merge ') || VERSION_COMMIT.test(commit.subject))
            return sections;
...
function extractSpecHighlights(spec) {
    ...
    const requirements = extractSection(spec.content, 'Requirements')
        .split('\n')
        .filter((line) => /^\|\s*REQ-[^|]+\|/.test(line))
        .map((line) => line.split('|')[2]?.trim())
        .filter((value) => Boolean(value));
    return [`### ${title}`, overview, ...requirements.map((requirement) => `- ${requirement}`)]
        .filter((value) => Boolean(value));
}
function renderCommitSections(sections) {
