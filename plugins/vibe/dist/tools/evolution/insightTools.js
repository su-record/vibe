...
// ============================================================================
export const extractInsightsDefinition = {
...
// ============================================================================
export const searchInsightsDefinition = {
...
export async function searchInsights(args) {
    try {
        ...
        const formatted = results.map((r, i) => `  ${i + 1}. [${r.type}] ${r.title} (confidence=${r.confidence.toFixed(2)}, occurrences=${r.occurrences}, status=${r.status})`).join('\n');
...
// ============================================================================
export const listSkillGapsDefinition = {
...
export async function listSkillGaps(args) {
    try {
        const storage = new MemoryStorage(args.projectPath || process.cwd());
        const store = new InsightStore(storage);
        ...
        const formatted = gaps.map((g, i) => `  ${i + 1}. ${g.title} (confidence=${g.confidence.toFixed(2)}, occurrences=${g.occurrences})`).join('\n');
...
// core_insight_stats
// ============================================================================
export const insightStatsDefinition = {
...
export async function insightStats(args) {
    try {
        const storage = new MemoryStorage(args.projectPath || process.cwd());
        const store = new InsightStore(storage);
...
//# sourceMappingURL=insightTools.js.map