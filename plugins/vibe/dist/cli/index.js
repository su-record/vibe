...
import { setupExternalLLM, removeExternalLLM, gptStatus, gptLogout, antigravityStatus, antigravityLogout, claudeStatus, claudeLogout, setZaiKey, setZaiCodingKey, zaiStatus, zaiLogout, } from './llm.js';
import { patchGlobalConfig, getVibeDir } from '../infra/lib/config/GlobalConfigManager.js';
import { init, setup, update, upgrade, remove, showHelp, showStatus, showVersion, telegramSetup, telegramChat, telegramStatus, telegramHelp, slackSetup, slackChannel, slackStatus, slackHelp, skillsAdd, figmaSetup, figmaStatus, figmaLogout, figmaBreakpoints, figmaHelp, configShow, configHelp, statsDefault, statsWeek, statsQuality, statsHelp, codexLaunch, codexStatus, codexShell, codexHelp, llmList, llmRefresh, llmHelp, } from './commands/index.js';
...
export { getProgressPath, loadProgress, saveProgress, initProgress, updatePhase, completeTask, recordCommit, incrementSession, formatProgressState, getProgressSummary, writeProgressText, } from '../infra/lib/IterationTracker.js';
...
function extractApiKey(command, subCommand) {
    return positionalArgs[2] || args.find(a => !a.startsWith('-') && a !== command && a !== subCommand);
}
function dispatchProviderCommand(command, subCommand, cfg) {
...
Example: vibe skills add vercel-labs/skills
    ...
    case 'codex': {
        ...
        if (args.includes('--setup')) {
            ...
            if (mIdx >= 0)
                model = args[mIdx + 1];
            if (codexSub?.startsWith('/'))
                model = codexSub.slice(1);
            const claudeArgs = args.slice(1).filter(a => a !== 'codex' && a !== '--model' && a !== model && !a.startsWith('/'));
