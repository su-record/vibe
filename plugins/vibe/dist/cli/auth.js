 ...
 */
export function getClaudeCodeStatus(checkAuth = false) {
    const status = { installed: false, version: null, authenticated: null };
    try {
    ...
    try {
        const result = execSync('claude -p "hi" --output-format json --max-budget-usd 0.001 --no-session-persistence', {
 ...
 * Claude Code 상태 포맷팅
 */
export function formatClaudeCodeStatus(status) {
    if (!status.installed) {
 ...
 * LLM 상태 포맷팅
 */
export function formatAuthMethods(auths, cliInstalled) {
    const parts = [];
 ...
 */
function formatLlmCliRow(installed, authenticated) {
    if (!installed)
