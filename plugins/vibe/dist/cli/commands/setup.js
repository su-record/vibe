...
// ============================================================================
function buildAuthOptions(status) {
    return [
        {
            value: 'claude-apikey',
...
async function executeAuthMethod(method) {
    switch (method) {
        ...
        case 'antigravity-apikey': {
            ...
            if (p.isCancel(key))
                return false;
            setupExternalLLM('antigravity', key);
            return true;
        }
 ...
 */
function validateActivation(status) {
    return (status.gpt.length > 0 ||
...
export async function setup() {
    ...
    if (!validateActivation(finalStatus)) {
        p.log.warn('No providers active. At least one of Claude, GPT, or Antigravity must be configured.');
