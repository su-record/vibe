...
import { updateConstitution, updateRules, migrateLegacyCore, consolidateLegacyVibe, updateGitignore, updateConfig, cleanupLegacy, removeLocalAssets, cleanupClaudeConfig, cleanupLegacyMcp, installProjectHooks, installProjectCodexHooks, installCodexNotify, generateProjectClaudeMd, generateProjectAgentsMd, generateProjectAntigravityMd, generateGlobalClaudeMd, generateGlobalCodexAgentsMd, generateGlobalAntigravityMd, } from '../setup.js';
import { installLocalSkills, installLanguageRules, } from './init.js';
 ...
 */
export function update(options = { silent: false }) {
    try {
        ...
        const primaryHarness = HARNESS_PRIORITY.find(d => fs.existsSync(path.join(projectRoot, d))) || '.claude';
        ...
        const presentHarnesses = PROJECT_HARNESS_DIRS.filter(d => fs.existsSync(path.join(projectRoot, d)));
