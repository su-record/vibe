 ...
 */
export function installLocalSkills(projectRoot, stackTypes, capabilities = [], harnessDir = '.claude') {
 ...
 */
export function installLocalAgents(projectRoot, stackTypes, capabilities = [], harnessDir = '.claude') {
 ...
 */
export function installLanguageRules(projectRoot, stackTypes, harnessDir = '.claude') {
 ...
 */
export async function init(projectName, target = 'cc') {
    try {
        const HARNESS_MAP = {
        ...
        if (stackDetails.capabilities.length === 0 && !process.env.CI && AVAILABLE_CAPABILITIES.length > 0) {
            ...
            if (!p.isCancel(selected) && Array.isArray(selected)) {
                const filtered = selected.filter(v => v !== '__none__');
        ...
        if (stackDetails.capabilities.includes('devlog') && !process.env.CI) {
            ...
            if (!p.isCancel(targetRepo)) {
                ...
                devlogConfig = {
                    ...
                    targetDir: p.isCancel(targetDir) ? 'posts' : (targetDir || 'posts'),
                    prefix: p.isCancel(prefix) ? 'devlog' : (prefix || 'devlog'),
