...
// Aggregation helpers
// ============================================================================
function aggregateSkillSummaries(events) {
    const map = new Map();
...
function computeOverallSuccessRate(events) {
...
function computeAvgDuration(events) {
...
function estimateSessionCount(events) {
    if (events.length === 0)
        return 0;
...
// Formatters
// ============================================================================
function formatSuccessRate(successCount, total) {
    if (total === 0)
...
// ============================================================================
function printSummarySection(events, decisions) {
...
export function statsDefault() {
    const events = readJsonlFile(SKILL_USAGE_PATH);
...
function printWeeklyBreakdown(events) {
    ...
    for (let i = 6; i >= 0; i--) {
        ...
        const dayEvents = events.filter(e => e.event_type === 'skill_run' && e.ts.startsWith(dateStr));
...
export function statsWeek() {
    const allEvents = readJsonlFile(SKILL_USAGE_PATH);
    const allDecisions = readJsonlFile(DECISIONS_PATH);
...
export function statsQuality() {
    const spans = readJsonlFile(SPANS_PATH);
    const decisions = readJsonlFile(DECISIONS_PATH);
