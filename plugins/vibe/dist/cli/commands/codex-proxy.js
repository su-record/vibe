...
import { launchSession, generateShellFunction, checkAuthSource, getProxySettings, } from '../../infra/lib/codex-proxy.js';
...
// ─── 실행 ────────────────────────────────────────────────────
export function codexLaunch(model, claudeArgs) {
    launchSession(model, claudeArgs);
}
// ─── 셸 함수 출력 ───────────────────────────────────────────
export function codexShell(modelArg) {
...
// ─── Setup ──────────────────────────────────────────────────
export async function codexSetup() {
...
async function configureAuth(provider) {
    switch (provider) {
        case 'chatgpt-pro':
...
// ─── Config 저장 ─────────────────────────────────────────────
function saveConfig(provider, auth, model, alias) {
    const patch = {
        codexProxy: {
...
// ─── 셸 등록 ─────────────────────────────────────────────────
async function registerShell(alias) {
    const profile = detectShellProfile();
    if (!profile) {
        p.log.warn('셸 프로파일 미탐지. 수동 등록 필요');
        return;
    }
...
function detectShellProfile() {
    ...
    if (shell.endsWith('/zsh') && fs.existsSync(zshrc))
        return zshrc;
    if (shell.endsWith('/bash') && fs.existsSync(bashrc))
        return bashrc;
...
// ─── 유틸 ────────────────────────────────────────────────────
async function textOrCancel(message, placeholder) {
