...
import { getProjectConfigPath, getProjectConfigPaths, } from '../../infra/lib/config/GlobalConfigManager.js';
/**
...
const SKILL_TARGET = new RegExp(`^(https:\\/\\/github\\.com\\/)?${OWNER_REPO}${SKILL_SUFFIX}$`);
 ...
 */
export function installExternalSkills(projectRoot, stackTypes, capabilities = []) {
    ...
    if (fs.existsSync(readConfigPath)) {
        try {
            const config = JSON.parse(fs.readFileSync(readConfigPath, 'utf-8'));
            installed = config.installedExternalSkills ?? [];
        }
        catch { /* ignore */ }
    }
    const toInstall = packages.filter((pkg) => !installed.includes(pkg));
