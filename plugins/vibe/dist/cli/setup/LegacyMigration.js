 ...
 */
export function consolidateLegacyVibe(projectRoot) {
    ...
    if (conflicts.length > 0) {
        ...
        if (conflicts.length > 10)
            log(`     ... and ${conflicts.length - 10} more`);
 ...
 */
function mergeDirInto(src, dst, srcRoot, conflicts) {
    ensureDir(dst);
    for (const entry of fs.readdirSync(src, { withFileTypes: true })) {
        ...
        if (entry.isDirectory()) {
            if (fs.existsSync(dstPath))
                mergeDirInto(srcPath, dstPath, srcRoot, conflicts);
            else
 ...
 */
export function migrateLegacyCore(projectRoot, coreDir) {
 ...
 * 빈 디렉토리는 정리.
 */
function removeCoreOwnedFiles(localDir, sourceDir) {
 ...
 */
export function removeLocalAssets(claudeDir, packageRoot) {
            ...
            for (const [projectPath, projectConfig] of Object.entries(claudeConfig.projects)) {
                if (projectConfig.mcpServers) {
                    if (projectConfig.mcpServers.core) {
                        ...
                        const isLocalPath = coreArgs.some((arg) => arg.includes('.core/mcp/') || arg.includes('.core\\mcp\\'));
                        if (isLocalPath) {
                    ...
                    if (projectConfig.mcpServers['core-gemini']) {
                        ...
                        const isLocalPath = legacyArgs.some((arg) => arg.includes('.core/') || arg.includes('.core\\'));
                        if (isLocalPath) {
 ...
 * 레거시 mcp/ 폴더 정리
 */
export function cleanupLegacyMcp(coreDir) {
...
//# sourceMappingURL=LegacyMigration.js.map