 ...
 */
export function missingNativeDeps(packageRoot) {
    const modules = join(packageRoot, 'node_modules');
    return NATIVE_DEPS
        .filter(({ name, artifact }) => existsSync(join(modules, name)) && !existsSync(join(modules, name, artifact)))
        .map(({ name }) => name);
}
/**
 * 빠진 바인딩을 내려받아 복구한다.
 ...
 *          "설치 성공" 이 또 거짓말이 된다.
 */
export function repairNativeDeps(packageRoot) {
    const modules = join(packageRoot, 'node_modules');
    ...
    for (const name of missingNativeDeps(packageRoot)) {
        ...
        try {
            if (!existsSync(prebuild))
                throw new Error(`prebuild-install not found: ${prebuild}`);
 ...
 */
export function nativeDepHint(names) {
