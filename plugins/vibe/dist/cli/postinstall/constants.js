 ...
 */
export const GLOBAL_SKILLS_ENTRY = [
    'vibe',
    'vibe.analyze',
    'vibe.continue',
    'vibe.contract',
...
export const GLOBAL_SKILLS_STANDARD = [
...
export const GLOBAL_SKILLS_OPTIONAL = [
    'vibe.commit-push-pr',
    'vibe.git-worktree',
    'vibe.tool-fallback',
 ...
 */
export function resolveDemotedGlobalSkills(shippedSkillNames) {
    const global = new Set(GLOBAL_SKILLS);
    return shippedSkillNames.filter(name => !global.has(name)).sort();
}
/** 스택 → 로컬 스킬 매핑 (vibe init/update → .claude/skills/) */
export const STACK_TO_SKILLS = {
 ...
 */
export const SKILL_ROOTS = ['skills', 'skills-extra'];
/** `skills-extra/` 에 있어야 하는 스킬 — optional + capability 전개 (파생값) */
export const EXTRA_SKILLS = [
    ...new Set([...GLOBAL_SKILLS_OPTIONAL, ...Object.values(CAPABILITY_SKILLS).flat()]),
].sort();
/** `skills/` 에 있어야 하는 공개 스킬 — 전역 + 스택 전개 (파생값) */
export const CORE_SKILLS = [
    ...new Set([...GLOBAL_SKILLS, ...Object.values(STACK_TO_SKILLS).flat()]),
].sort();
/** 스택 → 외부 스킬(skills.sh) 매핑 (vibe init/update → npx skills add) */
export const STACK_TO_EXTERNAL_SKILLS = {
 ...
 */
export const CAPABILITY_EXTERNAL_SKILLS = {
...
export const AVAILABLE_CAPABILITIES = [
    ...
    { value: 'education', label: 'Educational Content', hint: '강의·튜토리얼·워크숍 제작 (학습 목표 + 출처 추적)' },
 ...
 */
export function resolveLocalSkills(stackTypes, capabilities = []) {
    const skills = new Set(resolveByStack(STACK_TO_SKILLS, stackTypes));
    // Capability 기반 스킬
    for (const cap of capabilities) {
        const capSkills = CAPABILITY_SKILLS[cap];
        if (capSkills)
            capSkills.forEach(s => skills.add(s));
    }
    return [...skills];
}
/**
 ...
 */
export function resolveExternalSkills(stackTypes, capabilities = []) {
    const skills = new Set(resolveByStack(STACK_TO_EXTERNAL_SKILLS, stackTypes));
    for (const cap of capabilities) {
 ...
 */
export const CONDITIONAL_AGENT_GROUPS = ['ui', 'event'];
/** 스택 → 에이전트 그룹 매핑 (vibe init/update → .claude/agents/) */
export const STACK_TO_AGENT_GROUPS = {
    'typescript-react': ['ui'],
...
export const CAPABILITY_AGENT_GROUPS = {
 ...
 */
export const LEGACY_AGENT_GROUPS = [
    'figma',
];
/**
 ...
 */
export function resolveLocalAgentGroups(stackTypes, capabilities = []) {
    const groups = new Set(resolveByStack(STACK_TO_AGENT_GROUPS, stackTypes));
    for (const cap of capabilities) {
        const capGroups = CAPABILITY_AGENT_GROUPS[cap];
        if (capGroups)
            capGroups.forEach(g => groups.add(g));
    }
    return [...groups];
}
// ─── 스택 타입 → 언어 룰 파일 매핑 ───
...
export const LEGACY_SKILL_RENAMES = {
    'spec': 'vibe.spec',
    'test': 'vibe.test',
    ...
    'chub-usage': 'vibe.documentation-provider',
    'commerce-patterns': 'vibe.commerce-patterns',
    'commit-push-pr': 'vibe.commit-push-pr',
    'context7-usage': 'vibe.documentation-provider',
...
export const LEGACY_SKILL_HASHES = {
...
export const CLAUDE_MODEL_MAPPING = {
...
// 에이전트 → 도구 카테고리 매핑
export const CLAUDE_AGENT_TOOL_CATEGORY = {
    'architect': 'read-only',
...
// 에이전트 → 권한 모드 매핑
export const CLAUDE_AGENT_PERMISSION_MODE = {
    // 읽기 전용 에이전트 → plan
    'architect': 'plan',
    'code-reviewer': 'plan',
...
export const CLAUDE_AGENT_MEMORY = {
    'code-reviewer': 'user',
