...
import { resolveSkillRoots, getCoreConfigDir, ensureDir, copyDirRecursive, removeDirRecursive, copySkillsFiltered, removeLegacySkills, replaceTemplatesInDir, cleanupDuplicateSkillDirs, applyCodexSkillInvocationPolicies, cleanupOptionalSkills, cleanupRenamedSkills, } from './fs-utils.js';
...
import { generateGlobalClaudeMd, generateGlobalCodexAgentsMd, generateGlobalAntigravityMd, installCodexNotify, } from '../setup/ProjectSetup.js';
...
export function main() {
    try {
        ...
        function removeLegacyVibeCommands(cmdsDir) {
        ...
        function installCliAssets(targetDir, label) {
            ensureDir(targetDir);
            ...
            if (skillRoots.length > 0) {
                ...
                if (label === 'codex')
                    applyCodexSkillInvocationPolicies(sklsDir);
            }
        ...
        function installCoreAssets(targetDir) {
        ...
        try {
            ...
            if (detectAntigravityCli().installed)
                generateGlobalAntigravityMd();
        }
        catch (e) {
            console.warn('⚠️  global CLAUDE.md/AGENTS.md/GEMINI.md 갱신 실패:', e.message);
        }
        ...
        console.warn('⚠️  core postinstall warning:', error.message);
    }
}
