 ...
 */
export function injectEnvDefaults(packageRoot) {
 ...
 */
const GLOBAL_TOP_LEVEL_DEFAULTS = {
 ...
 */
export function cleanupGlobalSettingsHooks() {
    const globalClaudeDir = path.join(os.homedir(), '.claude');
    const globalSettingsPath = path.join(globalClaudeDir, 'settings.json');
    if (!fs.existsSync(globalSettingsPath)) {
        return;
 ...
 */
export function ensureGlobalEnvSettings() {
    const globalClaudeDir = path.join(os.homedir(), '.claude');
    const globalSettingsPath = path.join(globalClaudeDir, 'settings.json');
    try {
        // 디렉토리 확보
