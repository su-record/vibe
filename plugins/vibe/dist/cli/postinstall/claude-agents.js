...
import { CLAUDE_MODEL_MAPPING, CLAUDE_AGENT_TOOLS, CLAUDE_AGENT_TOOL_CATEGORY, CLAUDE_AGENT_PERMISSION_MODE, CLAUDE_AGENT_DISALLOWED_TOOLS, CLAUDE_AGENT_MEMORY, } from './constants.js';
/**
 * 에이전트별 description 생성
 */
function generateDescription(name, title, roleLines) {
 ...
 */
export function installClaudeAgents(agentsSource, claudeAgentsDir, options = {}) {
    ...
    let total = 0;
    let teamsSkipped = 0;
    function processDirectory(srcDir, destDir, relPath) {
        ensureDir(destDir);
        ...
        for (const entry of entries) {
            ...
            if (entry.isDirectory() && entry.name === 'teams' && relPath === '') {
                const teamCount = fs.readdirSync(srcPath, { recursive: true })
            ...
            if (relPath === '') {
                if (entry.isDirectory() && options.skipDirs?.includes(entry.name))
                    continue;
                if (options.onlyDirs && (!entry.isDirectory() || !options.onlyDirs.includes(entry.name)))
                    continue;
            }
            if (entry.isDirectory()) {
                ...
                try {
                    ...
                    console.warn(`   ⚠️ Failed to convert ${entry.name}: ${err.message}`);
                }
            }
        }
    }
