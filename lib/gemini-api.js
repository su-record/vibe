...
const GEMINI_MODELS = {
  ...
  'gemini-3-flash': {
    ...
    description: '최신 Flash 프리뷰, 빠른 응답',
    maxTokens: 8192,
  },
  // Gemini 3 Pro Preview (최신, 정확)
  'gemini-3-pro': {
    id: 'gemini-3-pro-preview',
    name: 'Gemini 3 Pro Preview',
    description: '최신 Pro 프리뷰, 높은 정확도',
    maxTokens: 8192,
  },
  // Gemini 2.5 Flash (Thinking 포함)
  'gemini-2.5-flash': {
    id: 'gemini-2.5-flash',
    name: 'Gemini 2.5 Flash',
    description: 'Thinking 기능 포함 Flash',
    maxTokens: 8192,
  },
...
// 기본 모델 (gemini-2.5-flash가 현재 가장 안정적)
const DEFAULT_MODEL = 'gemini-2.5-flash';

/**
 * 엔드포인트 URL 목록 (fallback 순서)
 * daily sandbox가 가장 먼저 시도됨
 */
 ...
 */
function wrapRequestBody(body, projectId, modelId, sessionId) {
  return {
    project: projectId,
    model: modelId,
...
async function chat(options) {
  ...
  // Gemini 3 모델은 thinkingConfig 필수
  if (modelInfo.id.includes('gemini-3')) {
    generationConfig.thinkingConfig = {
      thinkingLevel: 'low', // minimal, low, medium, high
      include_thoughts: true,
    };
  }
  // Gemini 2.5 모델도 thinkingConfig 지원
  else if (modelInfo.id.includes('gemini-2.5')) {
    generationConfig.thinkingConfig = {
      thinkingBudget: 1024, // 최소값
      include_thoughts: true,
    };
  }
