...
function audit(protocol, rows, task) {
  ...
  if (!calibration?.artifact || fileHash(calibration.artifact) !== calibration.sha256) throw new Error('human calibration evidence missing or changed');
}
...
try {
  if (args.includes('--self-test')) selfTest();
    ...
    const result = evaluate(protocol, rows, readLines(path.join(directory, 'reviews.jsonl')), rubric.requirements ?? rubric, readLines(path.join(directory, 'ci.jsonl')));
    ...
    console.log('4.1.26 release evidence passed: complete cohort, human reviews, per-cell quality and per-client cost targets, Linux/Windows CI');
  }
