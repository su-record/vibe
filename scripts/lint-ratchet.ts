...
// `.bin` 샴을 거치지 않고 oxlint 의 JS 엔트리를 node 로 직접 실행한다.
// 실측(Windows): `./node_modules/.bin/oxlint` 는 ENOENT, `.cmd` 는 EINVAL 로 죽는다
// — Node 는 CVE-2024-27980 수정 이후 `.cmd` 를 shell 없이 spawn 하지 않는다.
// shell: true 로 우회하는 대신 엔트리를 직접 부르면 플랫폼 분기가 필요 없다.
const OXLINT_ENTRY = join(REPO_ROOT, 'node_modules', 'oxlint', 'bin', 'oxlint');
...
function collectCounts(): Record<string, number> {
  let raw: string;
  try {
    raw = execFileSync(process.execPath, [OXLINT_ENTRY, '--format=json'], {
      cwd: REPO_ROOT,
      encoding: 'utf-8',
