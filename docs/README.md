...
Claude Code 전용 프레임워크. 바이브코딩을 쉽게, 최소 품질은 자동으로 보장합니다.
...
| AI 결과를 그대로 수용 | 4개 LLM 교차 검증 (Claude + GPT + Gemini + AZ (Kimi K2.5)) |
...
Azure Foundry에서 Kimi K2.5를 직접 호스팅하여 안정적인 성능을 제공합니다.

| 모델 | 용도 | 엔드포인트 |
|------|------|-----------|
| **Kimi K2.5** | 채팅/추론/코드 분석 (모든 태스크) | `fallingo-ai-foundry.services.ai.azure.com` |
| **text-embedding-3-large** | 임베딩 | `fallingo-ai-foundry.cognitiveservices.azure.com` |

동일한 API 키로 Chat과 Embedding 모두 사용 가능합니다.
...
| 작업 유형 | 우선순위 (폴백 체인) |
|----------|---------------------|
| 코드 분석, 리뷰, 추론 | AZ (Kimi K2.5) → GPT → Gemini → Claude |
| 아키텍처, 디버깅 | GPT → AZ (Kimi K2.5) → Gemini → Claude |
| UI/UX, 웹 검색 | Gemini → GPT → Claude |
| 코드 생성, 일반 | AZ (Kimi K2.5) → Claude |
...
# AZ (Azure Foundry)
vibe az key <KEY>        # AZ API 키 설정
vibe az status           # AZ 상태
vibe az logout           # AZ 로그아웃
...
작업 환경이 바뀔 때마다 GPT, Gemini, AZ 인증을 반복하는 문제를 해결합니다.
...
> 상세 설계 문서: [docs/auth-sync-design.md](auth-sync-design.md)
...
SmartRouter (작업 유형별 LLM 선택 + 폴백 체인)
    ...
    │   └→ AZ (Kimi K2.5 via Azure Foundry)
    │
